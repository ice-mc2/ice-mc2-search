...
// ====================================== message Sending Code ============================
	...
	$scope.searchEquation = function(source) {
		axios.get(SEARCH_API_URL + '/search?query=' + source.substring(2, source.length - 2))
			.then((data) => {
				let resultBody = "<div>";
				for(const item of data.data) {
					resultBody += `<h5>${item.title}</h5>`;
					resultBody += `<a href="${item.link}" target="_blank">View</a>`;
				}
				Alert.Alert.spawn("Search Result", "<pre><code>" + resultBody + "</code></pre>");
			});
	};
